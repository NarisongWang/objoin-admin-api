...
const uploadPdfFilesToAzure = asyncHandler(async (installationOrderNumber, localFilePath, files) =>{
...
const deleteInstallationOrderDirectoryFromAzure = asyncHandler(async(dir)=>{
    const blobServiceClient = BlobServiceClient.fromConnectionString(process.env.AZURE_STORAGE_CONN)
    const containerName = process.env.AZURE_STORAGE_CONTAINER
    const containerClient = blobServiceClient.getContainerClient(containerName)
...
// @acccess private function
// @params  dir: directory name
const findPdfFiles = asyncHandler(async (dir) =>{
    try {
        let files = []
